...
- `Rbtz.CredoChecks.Refactor.PreferWithOverCase`: Flags a two-clause `case` whose only non-happy-path clause re-returns an error shape unchanged — an `{:error, ...}` tuple or the bare `:error` atom (e.g. `{:error, reason} -> {:error, reason}`). A `with` expresses this more directly via its implicit pass-through `else`.

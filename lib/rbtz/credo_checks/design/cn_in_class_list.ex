defmodule Rbtz.CredoChecks.Design.CnInClassList do
  ...
  # Nested lists/strings/atoms/numbers only. `@assign`, slot access, variables,
  # and function calls may be caller-provided classes — leave those alone.
  ...
  defp find_class_attrs(heex, line_fn) do
    ...
    |> Enum.map(fn {offset, content} -> {line_fn.(offset), content} end)
  end
end
